...
export default function HeroSection({ data }: { data: Anime[] }) {
    const renderBadges = (item: Anime) => (
        <div className="flex flex-wrap items-center sm:justify-center gap-2 sm:gap-2.5">
            ...
            {item.status && (
                <Badge className="hidden md:flex px-3 py-1.5 bg-emerald-500/30 text-emerald-200 border border-emerald-400/40 rounded-full">
                    {item.status}
                </Badge>
            )}
            ...
            {item.episodes && (
                <Badge className="lg:hidden px-3 py-1.5 bg-purple-500/30 text-purple-200 border border-purple-400/40 rounded-full">
                            ...
                            <div className="absolute inset-0 flex flex-col justify-end items-center p-4 sm:p-6 lg:p-10 mx-auto">
                                <div className="max-w-4xl space-y-3 lg:space-y-5 text-center">
                                    <h2 className="text-white text-xl sm:text-3xl lg:text-4xl font-bold drop-shadow-xl">
                                        {item.title_english || item.title || item.title_japanese || "Unknown Title"}
                                    </h2>
