...
import AnimeCard from "../anime-components/moviecard";
...
export default function CarouselSlide({ data, type }: CarouselSlideProps) {
  ...
  useEffect(() => {
    function updateChunkSize() {
      ...
      if (width < 640) setChunkSize(2);       // grid-cols-2
      else if (width < 768) setChunkSize(3);  // sm:grid-cols-3
      else if (width < 1024) setChunkSize(4); // md:grid-cols-4
      else if (width < 1280) setChunkSize(5); // lg:grid-cols-5
      else if (width < 1536) setChunkSize(5); // xl:grid-cols-5
      else setChunkSize(6);                   // 2xl:grid-cols-6
    }
